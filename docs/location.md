...
On Windows, `WindowsLocationSource` wraps `Windows.Devices.Geolocation.Geolocator`. Permission is requested only from the foreground UI flow, because Windows requires `RequestAccessAsync` to run while the application is foregrounded and on the UI thread. The packaged-app location capability is a distribution concern and is not needed for the unpackaged development executable.
